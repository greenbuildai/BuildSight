...
class GeminiAuditor:
    ...
    def audit(self, img_bgr: np.ndarray, all_dets: list, condition: str = "normal"):
        """
        Run validation + supplementation in a single Gemini call.
        ...
        annotated, id_map = _draw_annotated(img_bgr, all_dets)

        # Build prompt
        prompt = self._build_prompt(all_dets, id_map, condition, w_img, h_img)

        # Call Gemini
        response_text = self._call_gemini(annotated, prompt)
        if response_text is None:
            # On failure: keep everything, add nothing
            return [], list(all_dets)

        # Parse response
        new_dets, kept_dets = self._parse_response(
            response_text, all_dets, id_map, w_img, h_img)
...
YOUR TASKS:
1. VALIDATE — For each labelled box, decide if it is a true detection.
   - A Worker box is valid only if it contains a visible human being (not a sign, shadow, statue, or machine).
   - A Helmet box is valid only if it is a hard hat / safety helmet worn on a human head.
   - A Vest box is valid only if it is a reflective hi-vis safety vest worn on a human body.

2. SUPPLEMENT — For each Worker box (W0, W1, …), check whether the person:
   - is wearing a helmet / hard hat that was NOT already detected (no yellow box covering their head).
   - is wearing a safety vest that was NOT already detected (no green box on their torso).
   If you see such PPE, provide the pixel bounding box [x1, y1, x2, y2].
   Only report PPE that you are confident about (>60% certainty).
   Do NOT report PPE on workers who do not have a valid Worker box.
