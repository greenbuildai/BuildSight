...
def profile_for_condition(condition: str) -> Dict[str, object]:
    profiles = {
        "S1_normal": {
            ...
            "post_gate": {CLS_HELMET: 0.22, CLS_VEST: 0.24, CLS_WORKER: 0.24},
            "recover_worker": 0.0,
        },
        "S2_dusty": {
            "pre_conf": 0.08,
            "wbf_iou": {CLS_HELMET: 0.42, CLS_VEST: 0.48, CLS_WORKER: 0.52},
            "post_gate": {CLS_HELMET: 0.18, CLS_VEST: 0.20, CLS_WORKER: 0.18},
            "recover_worker": 0.0,
        },
        "S3_low_light": {
            "pre_conf": 0.08,
            "wbf_iou": {CLS_HELMET: 0.40, CLS_VEST: 0.46, CLS_WORKER: 0.50},
            "post_gate": {CLS_HELMET: 0.16, CLS_VEST: 0.18, CLS_WORKER: 0.16},
            "recover_worker": 0.0,
        },
        "S4_crowded": {
            "pre_conf": 0.07,
            "wbf_iou": {CLS_HELMET: 0.38, CLS_VEST: 0.44, CLS_WORKER: 0.40},
            "post_gate": {CLS_HELMET: 0.14, CLS_VEST: 0.17, CLS_WORKER: 0.14},
            "recover_worker": 0.22,
        },
    }
...
def recover_crowded_workers(
    ...
    for cand in sorted(candidates, key=lambda item: -item["score"]):
        ...
        if ppe_support > 0 or neighbor_support > 0 or cand["score"] >= 0.42:
...
class TemporalPPEFilter:
    ...
    def update(self, boxes: List[Dict[str, object]], frame_index: int) -> List[Dict[str, object]]:
        ...
        for worker in workers:
            track = self.tracks.get(worker["track_id"])
            if track is None:
                static_filtered.append(worker)
                continue
            if track.hits >= 3 and track.human_score_ema < 0.36 and track.helmet_streak == 0 and track.vest_streak == 0:
                continue
            static_filtered.append(worker)
