...
class MaterialSuppressionLayer:
    ...
    def _gate1_geometry(self, box, frame_area):
        ...
        if h < self.min_worker_height_px:
            return True
        # Tightened 0.5 → 0.80: catches squat objects (cement bags, scaffold
        # crossmembers, buckets) that are almost as wide as they are tall.
        # Real standing workers have h/w ≥ 1.0 so this never fires for them.
        if w > 0 and (h / w) < 0.80:
            return True
        area = w * h
...
def is_valid_aspect(box, cls_name):
    ...
    if cls_name == "worker":
        # Tuned per user preference: 0.15 <= ratio <= 0.85
        # Workers are almost always strictly taller than wide (narrow).
        # Widening rejection (0.95 -> 0.85) to catch more scaffolding structures.
