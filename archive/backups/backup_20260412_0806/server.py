...
AUTO_CROWD_WORKER_THRESH = 4    # ≥ this many workers in a single frame → S4
AUTO_CROWD_OVERLAP_THRESH = 0.05  # mean pairwise IoU among workers → crowded cluster
AUTO_QUICK_WORKER_CONF = 0.22   # pre-pass confidence for rough worker count

CROWD_ENTER_FRAMES = 3   # frames of S4 signal needed to enter crowded mode
CROWD_EXIT_FRAMES  = 5   # frames of non-S4 signal needed to leave crowded mode
...
def classify_scene_fast(
    ...
    worker_boxes = [
        ...
        for b in (r.boxes or [])
        if int(b.cls[0]) in worker_cls_ids and float(b.conf[0]) >= AUTO_QUICK_WORKER_CONF
    ]
    ...
    if n_workers >= 3:
        # Check pairwise overlap for closely clustered workers
        overlaps = []
        for i in range(len(worker_boxes)):
            for j in range(i + 1, len(worker_boxes)):
                overlaps.append(iou_box(worker_boxes[i], worker_boxes[j]))
...
def is_valid_helmet(hbox: list, worker_bw: float, worker_bh: float) -> bool:
    ...
    cloth, shoulders, or body regions:
      • Aspect ratio 0.45–1.8 (helmets are roughly round; cap tightened 2.2→1.8
        to reject long kerchiefs and cloth wraps that score above the old limit)
    ...
    hh = max(hy2 - hy1, 1)
    aspect = hw / hh
    if aspect > 1.8 or aspect < 0.45:   # tightened 2.2→1.8: rejects cloth wraps
        return False
...
def is_valid_vest(vbox: list, worker_bw: float, _worker_bh: float = 0.0) -> bool:
    """
    Geometric sanity check for safety-vest detections.
    Rejects:
      • Aspect ratio > 3.5 — cement bags are very wide; vests are roughly square
        to 2:1 landscape on a torso.
      • Width > 90% of worker box width — a real vest can't be wider than
        the torso it's worn on (with a small margin for padding/loose fit).
      • Min size 10×10 px — below this = pixel noise.
    The vest height check is intentionally loose (no upper-bound) because vests
    extend from shoulder to waist and legitimately occupy 50–80% of worker height.
    """
    vx1, vy1, vx2, vy2 = vbox
    vw = max(vx2 - vx1, 1)
    vh = max(vy2 - vy1, 1)
    aspect = vw / vh
    if aspect > 3.5:                    # too landscape: cement/sand bag
        return False
    if vw > worker_bw * 0.90:           # wider than torso with margin
        return False
    if vw < 10 or vh < 10:              # pixel noise
        return False
    return True
    ...
    for wi, wd in workers:
        ...
        for vi, vd in vests:
            if vi in matched_vests:
                continue
            # Geometric sanity: reject cement bags / tarps masquerading as vests
            if not is_valid_vest(vd["box"], bw, bh):
                continue
...
def filter_orphan_ppe(detections: list) -> list:
    """
    Remove helmet and vest detections that have no spatially-nearby worker.

    After associate_ppe_to_workers(), unmatched PPE items (helmet/vest with no
    parent worker box) are left in the output and appear as floating annotations.
    These are nearly always false positives — kerchiefs on the ground, cement
    bags, fabric rolls, etc.

    A PPE item is kept only if at least one worker's expanded search zone
    contains the PPE centroid (same geometry as associate_ppe_to_workers).
    Workers are always kept regardless.
    """
    worker_ids = {k for k, v in cls_map.items() if v in ("worker", "person")}
    helmet_ids = {k for k, v in cls_map.items() if v == "helmet"}
    vest_ids   = {k for k, v in cls_map.items() if v in ("safety_vest", "safety-vest")}

    workers = [d for d in detections if d["cls"] in worker_ids]
    ...
    for det in detections:
        ...
        is_helmet = det["cls"] in helmet_ids
        ...
        for wd in workers:
            ...
            if is_helmet:
                ...
                if hx1 <= dcx <= hx2 and hy1 <= dcy <= hy2:
                    matched = True
                    break
            else:
                vx1, vx2 = bx1 - pad, bx2 + pad
                vy1, vy2 = by1, by2 + bh * 0.20
                if vx1 <= dcx <= vx2 and vy1 <= dcy <= vy2:
                    matched = True
                    break
...
def _zone_filter_detections(
    detections: list,
    zone_poly_norm: list[list[float]],
    img_w: int,
    img_h: int,
) -> list:
    """
    Server-side point-in-polygon zone filter.

    zone_poly_norm: list of [nx, ny] points in [0,1] letterbox-normalized space
    (same coordinate system as the frontend roiRef — centroid nx = cx/fw, ny = cy/fh).

    Detections whose centroid falls outside the polygon are removed.
    If the polygon has fewer than 3 points, all detections are kept unchanged.
    """
    if not zone_poly_norm or len(zone_poly_norm) < 3:
        return detections
    ...
    for det in detections:
        ...
        nx = cx / max(img_w, 1)
        ny = cy / max(img_h, 1)
        if _point_in_polygon(nx, ny, zone_poly_norm):
            kept.append(det)
    return kept
...
def _merge_full_and_tile(full_dets: list, tile_dets: list) -> list:
    ...
    for td in tile_dets:
        ...
        for fd in final:
            if fd["cls"] != td["cls"]:
                continue
            # Raised 0.35→0.45: same-person boxes from tile vs full-frame
            # frequently overlap 40–60% — they were not being deduped at 0.35.
            if iou_box(td["box"], fd["box"]) >= 0.45:
                duplicate = True
...
def _final_worker_nms(raw: list, iou_thresh: float = 0.45) -> list:
    """
    Final intra-class NMS applied to worker boxes AFTER all merging stages
    (WBF + tile + recovery).  Earlier stages may leave near-duplicate boxes
    when:
      - Two model predictions for the same person have IoU 0.40–0.54 (below
        the WBF merge threshold) and the tile pass adds another box.
      - recover_crowded_workers reintroduces a box that slipped through.

    Workers are sorted by confidence descending; any lower-scoring box that
    overlaps a kept box by ≥ iou_thresh is suppressed.
    Non-worker detections (helmet, vest) pass through unchanged — PPE overlap
    with workers is expected and must not be suppressed here.
    """
...
def run_inference(
    ...
    zone_poly:   list | None = None,   # [[nx,ny],...] in [0,1] letterbox-normalised coords
):
    ...
    if condition == "S4_crowded":
        wbf_iou_cond = {0: 0.45, 1: 0.50, 2: 0.55}
        base_nms_iou = 0.65
    elif condition == "S3_low_light":
    ...
    # ── Server-side zone filter (applied before expensive validation passes) ─
    # Removes detections whose centroid falls outside the operator-drawn ROI.
    # zone_poly is [[nx,ny],...] in [0,1] letterbox-normalised coords sent from
    # the frontend; if absent (None or <3 points) all detections pass through.
    if zone_poly and len(zone_poly) >= 3:
        raw = _zone_filter_detections(raw, zone_poly, w, h)
    ...
    raw = _final_worker_nms(raw, iou_thresh=0.45)
    ...
    # S4 elevated/distant workers score 0.25–0.38 after WBF and are cut at 0.40.
    # Clutter FPs at these scores are already blocked by ValidWorkerValidator,
    # MaterialSuppressionLayer, and is_valid_worker geometry checks above.
    if condition == "S4_crowded":
        effective_thresholds["worker"] = min(effective_thresholds["worker"], 0.28)
    ...
    # Remove floating PPE with no associated worker — almost always false positives
    final_detections = filter_orphan_ppe(final_detections)
...
async def detect_frame(
    ...
    zone_poly:     str   = Form(default=""),   # JSON [[nx,ny],...] ROI polygon or ""
):
    ...
    use_gemini_audit = gemini_audit not in ("0", "false", "False")
    parsed_zone: list | None = None
    if zone_poly:
        try:
            parsed_zone = json.loads(zone_poly)
            if not isinstance(parsed_zone, list) or len(parsed_zone) < 3:
                parsed_zone = None
    ...
    inference = await run_in_threadpool(
        ...
        cc, ni, wi, use_clahe, clahe_clip, use_gemini_audit, False, parsed_zone,
    )
