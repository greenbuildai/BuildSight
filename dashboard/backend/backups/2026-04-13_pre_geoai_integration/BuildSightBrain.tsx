...
const ORBITING_NODES = [
  { id: 'n1',  label: 'Detection Engine',    active: true, status: 'stable', controlGroup: 'thresholds' },
  { id: 'n2',  label: 'GeoAI',              active: true, status: 'stable', controlGroup: 'geoai' },
  { id: 'n3',  label: 'Turner AI',          active: true, status: 'stable', controlGroup: 'routing' },
  { id: 'n4',  label: 'Risk Engine',        active: true, status: 'warn',   controlGroup: 'sensitivity' },
  { id: 'n5',  label: 'Worker Tracking',    active: true, status: 'stable', controlGroup: 'tracking' },
  { id: 'n6',  label: 'PPE Compliance',     active: true, status: 'alert',  controlGroup: 'thresholds' },
  { id: 'n7',  label: 'Escalation',         active: true, status: 'stable', controlGroup: 'sensitivity' },
  { id: 'n8',  label: 'Site Intelligence',  active: true, status: 'stable', controlGroup: 'geoai' },
  { id: 'n9',  label: 'CCTV Streams',       active: true, status: 'stable', controlGroup: 'tracking' },
  { id: 'n10', label: 'Heatmaps',           active: true, status: 'warn',   controlGroup: 'geoai' },
  { id: 'n11', label: 'Zone Monitor',       active: true, status: 'stable', controlGroup: 'geoai' },
  { id: 'n12', label: 'Alert System',       active: true, status: 'stable', controlGroup: 'system' },
]
...
export function BuildSightBrain({ onBack }: BuildSightBrainProps) {
          ...
          <motion.div className="god-panel god-flex-grow" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
            ...
            {activeTab === 'geoai' && (
              <div className="god-tab-content">
                <div className="god-panel-header">
                  <h3>ADVANCED GEO-INTELLIGENCE</h3>
                </div>
                <div className="god-density-map" onClick={() => openControlsFor('geoai')}>
                  <div className="god-mock-heatmap" />
                  <div className="god-heatmap-overlay">LIVE HEATMAP</div>
                </div>
                <p className="god-meta" style={{ marginBottom: '16px' }}>
                  Dynamic worker-density heatmap. Sector 4 at 84% spatial density. Crane swing overlap risk: HIGH.
                </p>
                <div className="god-safety-metrics">
                  <div className="god-metric-row" onClick={() => openControlsFor('geoai')}>
                    <span>Zone Alpha Status</span>
                    <strong style={{ color: '#00ff80' }}>CLEAR</strong>
                  </div>
                  <div className="god-metric-row" onClick={() => openControlsFor('geoai')}>
                    <span>Zone Beta Status</span>
                    <strong style={{ color: '#ffaa00' }}>RESTRICTED</strong>
                  </div>
                  <div className="god-metric-row" onClick={() => openControlsFor('geoai')}>
                    <span>Crane Swing Radius</span>
                    <strong style={{ color: '#ff2a2a' }}>VIOLATION</strong>
                  </div>
                  <div className="god-metric-row" onClick={() => openControlsFor('geoai')}>
                    <span>Site Sector Occupancy</span>
                    <strong>142 / 200</strong>
                  </div>
                </div>
              </div>
            )}
