...
def _worker_has_ppe(worker_box: List[float], ppe_boxes: List[Dict[str, object]], cls_id: int) -> bool:
    """
    Return True if any PPE box of the given class is spatially associated with
    the worker box.

    For helmets (cls_id == CLS_HELMET): applies a vertical constraint — the
    helmet centroid must be in the upper 40 % of the worker box (+ 10 % headroom
    above).  This prevents wrist/palm/low-body detections from being counted as
    helmet confirmations, which would incorrectly increment helmet_streak.

    For vests (cls_id == CLS_VEST): standard full-height containment with a
    20 % extension below the box bottom (vest often extends below tight boxes).
    """
    wx1, wy1, wx2, wy2 = worker_box
    worker_h = max(wy2 - wy1, 1.0)
    bw = max(wx2 - wx1, 1.0)
    pad = bw * 0.15
    ...
    for det in ppe_boxes:
        ...
        if cls_id == CLS_HELMET:
            # Vertical constraint: centroid must be in upper 40% + 10% headroom above
            hy_top = wy1 - worker_h * 0.10
            hy_bot = wy1 + worker_h * 0.40
            if wx1 - pad <= cx <= wx2 + pad and hy_top <= cy <= hy_bot:
                return True
        else:
            # Vest: full worker height + 20% extension below
            if (wx1 - pad <= cx <= wx2 + pad
                    and wy1 <= cy <= wy2 + worker_h * 0.20):
                return True
    return False
...
def _copy_ppe_from_track(worker_box: List[float], cls_id: int, score: float) -> Dict[str, object] | None:
    """
    Synthesise a PPE box at the expected anatomical position for a worker whose
    PPE was confirmed in a recent frame but is not detected in the current one.

    Helmet placement: centred horizontally, occupying the top ~22 % of the worker
    box height with a 4 % headroom above.  This keeps the synthetic box inside the
    upper-40 % zone enforced by _worker_has_ppe, so it won't be immediately
    discarded by the vertical constraint on the next frame.

    Vest placement: covers the torso region (18–62 % of worker height), leaving
    head and leg regions clear.
    """
    x1, y1, x2, y2 = worker_box
    w = x2 - x1
    h = y2 - y1
    if cls_id == CLS_HELMET:
        # Centre horizontally (28–72 % of width), top of head zone
        return {
