...
AUTO_CROWD_WORKER_THRESH   = 6     # ≥ N high-conf valid workers → S4 signal
...
CROWD_ENTER_FRAMES         = 2     # consecutive S4 signals needed to enter crowded
CROWD_EXIT_FRAMES          = 3     # consecutive non-S4 signals needed to leave crowded
...
def is_valid_worker(
    ...
    if model_count == 1 and not nearby_ppe:
        # Increase required score for orphaned portrait blobs (cement bags)
        if score < 0.55:
            return False
