...
function VideoUploadMode() {
  ...
  const drawOverlay = useCallback(() => {
    ...
    for (const t of tracksRef.current) {
      ...
      // ── Tiered Compliance Coloring Strategy ────────────────────────────────
      // Blue (#0088ff): Fully Compliant (Helmet + Vest)
      // Yellow (#ffaa00): Partial PPE (One missing)
      // Red (#ff2a2a): No PPE / Critical Violation
      const isWorkerBox = t.cls === 'worker' || t.cls === 'person'
      let borderCol = col
      
      if (isWorkerBox) {
        if (t.has_helmet && t.has_vest) {
          borderCol = '#0088ff' // Brand Blue: Full Compliance
        } else if (t.has_helmet || t.has_vest) {
          borderCol = '#ffaa00' // Amber: Partial PPE
        } else {
          borderCol = '#ff2a2a' // Red: Critical Violation
        }
      }
...
export function LiveMode() {
  ...
  const drawOverlay = useCallback(() => {
    ...
    for (const t of tracksRef.current) {
      ...
      // ── Tiered Compliance Coloring Strategy (Synced with VideoUploadMode) ──
      const col = resolveCssVar(CLASS_COLORS[t.cls] ?? '#aaaaaa')
      ...
      if (isWorkerBoxLive) {
        if (t.has_helmet && t.has_vest) {
          ...
          borderColLive = '#ffaa00' // Amber: Partial PPE
        } else {
          borderColLive = '#ff2a2a' // Red: Critical Violation
        }
      }
