...
POST_WBF_BY_CONDITION: dict[str, dict[int, float]] = {
    ...
    # S4 worker gate raised 0.15→0.20: at 0.15 cement bags, wooden poles, and
    # scaffolding cross-members that score 0.15–0.19 were surviving into the
    # final result.  Genuine small/distant workers typically score ≥ 0.22 after
    # two-model WBF because both models weakly agree; clutter tends to be
    # single-model (already gated in is_valid_worker) or very low confidence.
    "S4_crowded":   {0: 0.28, 1: 0.12, 2: 0.20},
}
...
def is_valid_worker(
    ...
    if model_count == 1:
        if condition == "S3_low_light":
            ...
            # Raised 0.35→0.40: solo-model detections in crowded scenes are a
            # primary source of clutter FPs (bucket, bag, plank at conf 0.25–0.38).
            # Real occluded workers that only one model sees tend to score ≥ 0.40.
            single_thresh = 0.40
        else:
    ...
    if condition == "S1_normal":
        ...
        # Tightened: 1.80→1.60, score gate 0.58→0.62.
        # Elevated wall workers have aspect ≤ 1.4 from below-camera angles;
        # material piles and planks commonly land at 1.6–2.4 at low confidence.
        if aspect > 1.60 and score < 0.62:
            return False
