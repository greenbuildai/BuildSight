...
def wbf_fuse_condition(
    ...
    if condition == "S4_crowded":
        ...
        refined = _recover_crowded_workers(
            refined, all_preds, profile["recover_worker"]
        )
    ...
    for boxes, scores, labels in all_preds:
        for box, score, label in zip(boxes, scores, labels):
            if int(label) == CLS_WORKER and float(score) >= min_score:
                candidates.append({"box": list(box), "score": float(score), "cls": CLS_WORKER, "n_models": 1})

    for cand in sorted(candidates, key=lambda x: -x["score"]):
        if any(_iou(cand["box"], e["box"]) >= 0.65 for e in workers + recovered):
            continue
        bw, bh = cand["box"][2]-cand["box"][0], cand["box"][3]-cand["box"][1]
        if bh < 24 or bw / max(bh, 1.0) > 0.90:
            continue
        ppe_support = sum(
            1 for d in ppe
            if cand["box"][0] <= (d["box"][0]+d["box"][2])/2 <= cand["box"][2]
            and cand["box"][1] <= (d["box"][1]+d["box"][3])/2 <= cand["box"][3]
        )
        neighbor_support = sum(1 for w in workers if _iou(cand["box"], w["box"]) >= 0.02)
...
class TemporalPPEFilter:
    ...
    def update(self, dets: List[Dict], frame_index: int) -> List[Dict]:
        ...
        for w in workers:
            ...
            if t.hits < 2 and not has_ppe and w["score"] < 0.72:
                continue

            # Human-score gate
            if (t.hits >= 3 and t.human_score_ema < 0.40
                    and t.helmet_streak == 0 and t.vest_streak == 0):
                continue

            # Static-object gate (cement bags, scaffolding, machinery)
            if (t.is_static() and w["score"] < 0.80
                    and t.helmet_streak == 0 and t.vest_streak == 0):
                continue
