...
RECOVERY_COOLDOWN_FRAMES = 4   # skip recovery for this region for N frames
...
def recover_crowded_workers(
    ...
    for cand in sorted(candidates, key=lambda item: -item["score"]):
        # Raised 0.55→0.65: same-person boxes from two models have IoU 0.55–0.65.
        # At 0.55 the recovery was reintroducing boxes that WBF intentionally
        # left un-merged, creating 2–4 stacked boxes per worker.
        if any(iou(cand["box"], existing["box"]) >= 0.65 for existing in workers + recovered):
            continue
        ...
        # Require PPE support OR both high confidence AND neighbor support.
        # Removed bare neighbor_support path — a bucket next to a real worker
        # was generating false recoveries via neighbor_support alone.
        if ppe_support > 0 or (neighbor_support > 0 and cand["score"] >= 0.52) or cand["score"] >= 0.56:
            # Per-region cooldown: skip if this grid cell fired recently
            ...
            if (_recovery_global_frame - last_fired) < RECOVERY_COOLDOWN_FRAMES:
                continue  # cooldown active — don't re-trigger in this region
...
class TemporalPPEFilter:
    ...
    def update(self, boxes: List[Dict[str, object]], frame_index: int) -> List[Dict[str, object]]:
        ...
        for worker in workers:
            ...
            # frames before it is displayed.  Single-frame detections (ghosts) are
            # overwhelmingly clutter or noise — cement bags occasionally trigger on
            # exactly 1 frame then disappear.  Workers confirmed by PPE bypass this.
            has_ppe = track.helmet_streak > 0 or track.vest_streak > 0
            if track.hits < 2 and not has_ppe and worker["score"] < 0.72:
                continue

            # Human-score gate (hits≥3, low EMA, no PPE history)
            if (track.hits >= 3
                    and track.human_score_ema < 0.40
            ...
            # Tightened variance: 3.5→3.0 px² — stricter static detection.
            # Allow exception when confidence is very high (≥0.80).
