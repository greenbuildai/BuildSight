...
function VideoUploadMode() {
  ...
  const drawOverlay = useCallback(() => {
    ...
    // α=0.50 per RAF tick: boxes reach the new position faster than α=0.40,
    ...
    for (const t of tracksRef.current) {
      ...
      // Brand blue for fully compliant workers; class colour otherwise
      const isWorkerBox = t.cls === 'worker' || t.cls === 'person'
      const isCompliant = isWorkerBox && t.has_helmet === true && t.has_vest === true
      const col = isCompliant
        ? '#0088ff'
        : resolveCssVar(CLASS_COLORS[t.cls] ?? '#aaaaaa')
...
export function LiveMode() {
  ...
  const drawOverlay = useCallback(() => {
    ...
    const LERP = 0.50  // faster snap to new position reduces perceived lag
