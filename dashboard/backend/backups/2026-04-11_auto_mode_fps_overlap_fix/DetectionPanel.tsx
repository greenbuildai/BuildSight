    ...
    const loop = async () => {
      ...
      try {
        ...
        if (res.ok && isRunningRef.current) {
          ...
          // Update detected condition from auto-classifier response
          if (data.condition) setDetectedCondition(data.condition)

          // Smoothed FPS over last 5 frames
          if (frameElapsed > 0) {
            const hist = fpsHistoryRef.current
            hist.push(Math.round(1000 / frameElapsed))
            if (hist.length > 5) hist.shift()
            setLiveFps(Math.round(hist.reduce((a, b) => a + b, 0) / hist.length))
          }

          // Record peak risk moments
        ...
        <div className="det-risk-sidebar">
          ...
          <div className="det-risk-sidebar__list">
            {peakRiskMoments.length === 0 ? (
              <div className="det-risk-sidebar__empty">No high-risk moments detected yet.</div>
            ) : (
              peakRiskMoments.map((moment, idx) => (
                ...
                >
                  <div className="det-risk-item__time">{_fmtTime(moment.time)}</div>
                  <div className="det-risk-item__meta">
                    <span className="det-risk-item__label">{moment.type}</span>
                    <span className="det-risk-item__score">Risk Level: {moment.score}</span>
                  </div>
                </button>
              ))
            )}
