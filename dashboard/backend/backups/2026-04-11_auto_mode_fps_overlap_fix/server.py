...
class SceneConditionTracker:
    ...
    Thread-safety: the /detect/frame endpoint runs in run_in_threadpool.
    Access is protected by a simple counter approach (no threading.Lock needed
    since FastAPI's threadpool serialises calls per session on the RTX 4050).
    """

    def __init__(self):
        self.current: str = "S1_normal"         # last stabilised condition
        self._history: list[str] = []           # rolling window of raw readings
        self._crowd_consecutive: int = 0        # frames with S4 signal
        self._non_crowd_consecutive: int = 0    # frames without S4 signal
    ...
    def update(self, raw_condition: str) -> str:
        ...
        if self.current == "S4_crowded":
            # Already in crowded — exit only after sustained non-crowded signal
            if self._non_crowd_consecutive >= CROWD_EXIT_FRAMES:
                self.current = self._stabilise_non_crowd()
        else:
            # Not in crowded — enter after sustained crowded signal
    ...
    def _stabilise_non_crowd(self) -> str:
        """Majority vote over the last STABILITY_WINDOW non-S4 readings."""
    ...
    def reset(self):
        """Reset tracker — call when a new video is loaded."""
        self.__init__()
...
def is_valid_worker(
    ...
    # ── Signal 7: height floor ─────────────────────────────────────────────
    # S4_crowded: 0.06→0.045 — workers on top of walls appear small because
    #   the camera is below them; their bounding boxes are shorter than a
    #   ground-level worker at the same horizontal distance.
    ...
    if model_count == 1:
        if condition == "S3_low_light":
            ...
            # Lowered 0.40→0.35: in crowded scenes one model often misses
            # due to occlusion; the other model's solo detection is still valid.
            single_thresh = 0.35
        else:
    ...
    #   S4_crowded:  relaxed — overlapping workers produce wide boxes AND
    #                elevated workers have compressed height from top-down view.
    if condition == "S1_normal":
        ...
        # Only reject very wide landscape boxes with low confidence.
        # Elevated workers on walls appear foreshortened; their aspect ratio
        # can be as wide as 1.4. Only reject above 1.8 unless score is solid.
        if aspect > 1.80 and score < 0.58:
            return False
...
def _looks_suspicious_for_gemini(
    ...
    score = float(det.get("score", 0.0))
    if score < GEMINI_AUDIT_MIN_CONF or score > GEMINI_AUDIT_MAX_CONF:
        return False
...
def run_inference(
    ...
    use_tiling:  bool  = False,        # enable tile-based recall for crowded scenes
):
    audit_img_bgr = img_bgr.copy()

    # ── Auto scene classification ─────────────────────────────────────────────
    # When condition == "auto", classify this frame and run it through the
    # hysteresis tracker to get a stable condition string.
    # The resolved condition is returned in position [3] of the tuple so the
    # endpoint can echo it to the frontend for the live mode indicator.
    if condition == "auto":
    ...
    # ── Tile-based recall (S4_crowded or explicit use_tiling) ────────────────
    # Runs a 2×2 overlapping grid pass to recover workers that the full-frame
    # pass misses: elevated workers on walls, edge workers, and small distant
    # workers whose features are too fine to trigger at full resolution.
    # Only runs when condition is crowded (or caller forces it) and both models
    # are loaded — tiling doubles inference calls so we skip it for S1/S2/S3
    # where its benefit is minimal and latency budget is tighter.
    if use_tiling or condition == "S4_crowded":
        ...
        used_ensemble = True  # tiling always runs both models when available
    ...
    raw, audit_meta = apply_gemini_audit(audit_img_bgr, raw, condition, gemini_audit)
    raw = associate_ppe_to_workers(raw)

    # Return the resolved condition so endpoints can echo it to the frontend.
    # When the caller passed a hard-coded condition string, auto_resolved=False
    # and the condition value is unchanged — callers can always trust it.
...
async def detect_image(
    ...
    detections, used_ensemble, audit_meta, resolved_condition = await run_in_threadpool(
    ...
    def _process() -> tuple[str, int, float]:
        ...
        while True:
            ...
            if frame_idx % sample_every == 0:
                raw, used, _, _cond = run_inference(frame, condition, confidence, gemini_audit=use_gemini_audit)
