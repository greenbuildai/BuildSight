...
def apply_advanced_dust(image_path, output_path):
    ...
    dust_mask = (dust_mask - dust_mask.min()) / (dust_mask.max() - dust_mask.min())
...
# Test on the same file the user pointed out
src_folder = r"E:\Company\Green Build AI\Prototypes\BuildSight\Dataset\Indian Dataset\Normal_Site_Condition"
sample_file = "IMG_20160519_173042454.jpg" # Trying to find original of the SD one
if not os.path.exists(os.path.join(src_folder, sample_file)):
    files = [f for f in os.listdir(src_folder) if f.endswith('.jpg')]
    sample_file = files[0]

apply_advanced_dust(os.path.join(src_folder, sample_file), r"E:\Company\Green Build AI\Prototypes\BuildSight\sample_advanced_dust.jpg")
print(f"Generated sample at sample_advanced_dust.jpg using {sample_file}")
