...
Run: python buildsight_3d_v4.py
"""
...
# Camera marker
# Camera body box (visible 3D block)
traces.append(box_mesh(
    CAM_X-0.6, CAM_Y-0.4, CAM_Z-0.3,
    CAM_X+0.6, CAM_Y+0.4, CAM_Z+0.3,
    '#2471A3', 'Camera body', opacity=0.95, show_legend=False
))
# Camera lens cone (small cone pointing south/downward toward site)
traces.append(go.Cone(
    x=[CAM_X], y=[CAM_Y], z=[CAM_Z],
    u=[0], v=[-1], w=[-0.5],
    sizemode='absolute', sizeref=2.5,
    colorscale=[[0,'#1A5276'],[1,'#5DADE2']],
    showscale=False, opacity=0.90,
    name='📷 CAM-01 lens direction',
    hoverinfo='skip', showlegend=False
))
traces.append(go.Scatter3d(
    x=[CAM_X],y=[CAM_Y],z=[CAM_Z+1.0],
    mode='markers+text',
    marker=dict(size=16,color='#2471A3',symbol='diamond',
                line=dict(color='white',width=3)),
    text=['📷 CAM-01  25ft / 7.62m'],
    textposition='top center',
    textfont=dict(size=13,color='#154360',family='Arial Black'),
...
fig.show()
fig.write_html("buildsight_3d_v4.html")
print("Saved: buildsight_3d_v4.html")
